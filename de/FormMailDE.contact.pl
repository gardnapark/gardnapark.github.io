...
BEGIN
{
  ...
  $postmaster        = 'formmail@gardnapark.pl';
  @referers          = ();
  @allow_mail_to     = 'biuro@gardnapark.pl';
  @recipients        = ();
  %recipient_alias   = ();
...
<p>
  ...
  like this:
  <tt>&lt;form action=&quot;/cgi-bin/FormMail.contact.pl&quot; method=&quot;post&quot;&gt;</tt>
</p>
END
    return 0;
  }
...
sub send_main_email_header {
  ...
  my $subject = $self->{FormConfig}{subject} || 'Thank You';
  if ($self->{CFG}{secure}) {
...
sub success_page_html_preamble {
  ...
  my $title = $self->escape_html( $self->{FormConfig}{'title'} || 'Geschickt' );
...
END
  ...
  <body style="font-family: 'Lora', serif;" $attr>
    <h1 class="title">Nachricht gesendet</h1>
	<h2 class="title nav justify-content-center">Danke!</h2>
	<hr size="1" width="75%" />
	<div class="nav justify-content-center">
	
	<a class="btn btn-success" href="http://www.gardnapark.pl/de/index.html">Zurück zur Startseite Gardna Park</a>
	</div>
